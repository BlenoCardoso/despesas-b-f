...
export function SettingsPage() {
  ...
  const renderAboutSection = () => (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">LGPD e Privacidade</h3>

        <div className="space-y-4">
          <p className="text-sm text-gray-600">Baixe uma cópia dos seus dados pessoais ou solicite exclusão conforme a Lei Geral de Proteção de Dados (LGPD).</p>
          <button
            onClick={() => {
              exportData.mutate('all')
              toast.success('Gerando pacote de dados...')
            }}
            disabled={exportData.isPending}
            className="w-full flex items-center justify-between p-4 border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50"
          >
            <div className="text-left">
              <h4 className="font-medium text-gray-900">Exportar meus dados (LGPD)</h4>
              <p className="text-sm text-gray-500">Receba um arquivo com seus dados pessoais e atividades</p>
            </div>
            {exportData.isPending ? (
              <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-600"></div>
            ) : (
