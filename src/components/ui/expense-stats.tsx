...
export function ExpenseStats({ expenses }: ExpenseStatsProps) {
  const stats = useMemo(() => {
    ...
    const parseDate = (d: any) => new Date((d as any)?.toDate?.() ? (d as any).toDate() : d)

    const currentMonthExpenses = expenses.filter(expense => {
      const expenseDate = parseDate((expense as any).date)
    ...
    const getAmount = (e: any) => Number((e as any).amount ?? 0)
    const getTitle = (e: any) => String((e as any).title ?? (e as any).description ?? '')

    const currentTotal = currentMonthExpenses.reduce((sum, expense) => sum + getAmount(expense), 0)
    const lastTotal = lastMonthExpenses.reduce((sum, expense) => sum + getAmount(expense), 0)
    ...
    const categoryData = Object.entries(categoryLabels).map(([key, label]) => {
      const total = currentMonthExpenses
        .filter(expense => {
          const title = getTitle(expense).toLowerCase()
          switch (key) {
            case 'food':
      ...
      return {
        name: label,
        value: total,
        color: (categoryColors as any)[key as keyof typeof categoryColors]
      }
    ...
    const dailyData = last7Days.map(date => {
      ...
      const total = dayExpenses.reduce((sum, expense) => sum + getAmount(expense), 0)
